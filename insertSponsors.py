...
def insert_sponsors_from_json(json_path, bill_id_filter=None):
    try:
        ...
        for bill in bills_data:
            bill_id = bill.get("billId")  # Make sure to match the correct key
            sponsors = bill.get("sponsors")

            if bill_id_filter and bill_id != bill_id_filter:
                print("here")
                continue  # Skip bills that don't match the filter

            if bill_id and sponsors:
                response = supabase.table("enhanceddata").update({
                    "sponsors": sponsors  # Supabase should auto-serialize JSON properly
